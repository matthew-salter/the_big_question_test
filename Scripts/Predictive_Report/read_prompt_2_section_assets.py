...
def run_prompt(data):
    try:
        ...
        supabase_path = f"Predictive_Report/Ai_Responses/Prompt_2_Section_Assets/{run_id}.txt"
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                return {
                    "status": "success",
                    "run_id": run_id,
                    "prompt_2_section_assets": flattened
                }
        ...
        return {
            "status": "error",
            "run_id": run_id,
            "message": "Prompt 2 Section Assets file not yet available. Try again later."
        }

    except Exception as e:
        logger.exception("Unhandled error in read_prompt_2_section_assets")
        return {
            "status": "error",
